...
def manage_build_reading_list():
    """ Manages db-querying, assembling, and posting reading-list files to output directory. 
        Called by if-main. """
    ## settings -----------------------------------------------------
...
def load_initial_settings() -> dict:
    ...
    log.debug( f'OIT courses filepath, ``{settings["COURSES_FILEPATH"]}``' )
    log.debug( f'FILES_URL_PATTERN, ``{settings["FILES_URL_PATTERN"]}``' )
    return settings
...
if __name__ == '__main__':
    log.debug( 'starting if-main' )
    manage_build_reading_list()
